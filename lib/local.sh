...
provision() {
  ...
  ssh $SSH_OUTPUT_FLAG -tt "$SERVER_HOSTNAME" "export SERVER_HOSTNAME=$SERVER_HOSTNAME; export VERBOSE=$VERBOSE; cd $PROVISIONER_REMOTE_ROOT && bash $SERVER_PROVISION_FILE 2>&1 | tee -a /var/log/provisioner.log"
}
